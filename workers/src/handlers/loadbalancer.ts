...
export async function createLoadBalancerHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'create_load_balancer', 'loadbalancer', {
      lbId: result.lb?.id,
      name: body.name,
...
export async function updateLoadBalancerHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'update_load_balancer', 'loadbalancer', {
      lbId: id,
      updates: body,
    })
  }
...
export async function deleteLoadBalancerHandler(c: Context<{ Bindings: Env }>) {
  ...
  await logAudit(c, user?.id, 'delete_load_balancer', 'loadbalancer', {
    lbId: id,
  })
...
export async function addTargetHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'add_lb_target', 'loadbalancer', {
      lbId: id,
...
export async function removeTargetHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'remove_lb_target', 'loadbalancer', {
      lbId: id,
      targetId,
    })
  }
...
export async function updateTargetWeightHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'update_target_weight', 'loadbalancer', {
      lbId: id,
      targetId,
      weight: body.weight,
    })
...
export async function toggleLoadBalancerHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'toggle_load_balancer', 'loadbalancer', {
      lbId: id,
      enabled: !lb.enabled,
    })
  }
