...
export async function createScalingPolicyHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'create_scaling_policy', 'autoscaling', {
...
export async function updateScalingPolicyHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'update_scaling_policy', 'autoscaling', {
      policyId: id,
      updates: body,
    })
  }
...
export async function deleteScalingPolicyHandler(c: Context<{ Bindings: Env }>) {
  ...
  await logAudit(c, user?.id, 'delete_scaling_policy', 'autoscaling', {
    policyId: id,
  })
...
export async function executeScalingActionHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'execute_scaling_action', 'autoscaling', {
      policyId: id,
...
export async function runScalingCheckHandler(c: Context<{ Bindings: Env }>) {
  ...
  await logAudit(c, user?.id, 'run_scaling_check', 'autoscaling', {
...
export async function toggleScalingPolicyHandler(c: Context<{ Bindings: Env }>) {
  ...
  if (result.success) {
    await logAudit(c, user?.id, 'toggle_scaling_policy', 'autoscaling', {
      policyId: id,
      enabled: !policy.enabled,
    })
  }
